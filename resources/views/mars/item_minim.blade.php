                                ...
                                <tr>
                                    ...
                                    <th>
                                        <div class="filter-header">
                                            ...
                                            <select class="form-select form-select-sm filter-select" data-column="6"
                                                style="margin-top: 5px;">
                                                <option value="">All</option>
                                            </select>
                                        </div>
                                    ...
                                    <th>
                                        <div class="filter-header">
                                            <span>User</span>
                                            <select class="form-select form-select-sm filter-select" data-column="12"
                                                style="margin-top: 5px;">
                                                <option value="">All</option>
                                            </select>
                                        </div>
                                    ...
                                    <th>
                                        <div class="filter-header">
                                            ...
                                            <select class="form-select form-select-sm filter-select" data-column="16"
                                                style="margin-top: 5px;">
                                                <option value="">All</option>
                                            </select>
                                        </div>
                                    ...
                                    <th>
                                        <div class="filter-header">
                                            ...
                                            <select class="form-select form-select-sm filter-select" data-column="17"
                                                style="margin-top: 5px;">
                                                <option value="">All</option>
                                            </select>
                                        </div>
                            ...
                            <tbody>
                                @forelse($minimItems as $index => $item)
                                    ...
                                    <tr class="{{ !empty($item['duplicate_note'] ?? null) ? 'table-warning' : '' }}"
                                        ...
                                        <td>
                                            @if(auth()->check() && auth()->user()->username !== 'guest' && (auth()->user()->isPurchasing() || auth()->user()->isMaster() || in_array(auth()->user()->username, ['purchasing', 'master', 'admin'])))
                                                <button type="button" class="btn btn-sm btn-primary open-follow-modal-btn"
                                                    data-item-id="{{ $item['id'] }}" data-item-code="{{ $item['item_code'] ?? '' }}"
                                                    data-item-name="{{ $item['item_name'] ?? '' }}"
                                                    data-outstanding="{{ $item['outstanding'] ?? 0 }}"
                                                    data-request-whc="{{ $requestWhc !== null ? $requestWhc : '' }}"
                                                    data-request-whc-date="{{ !empty($item['request_whc_date']) ? \Carbon\Carbon::parse($item['request_whc_date'])->format('d/m/Y') : '' }}"
                                        ...
                                        <td>
                                            ...
                                            @if(!empty($poData))
                                                @if($hasMultiplePO)
                                                    <select class="form-select form-select-sm po-select" data-item-id="{{ $item->id }}"
                                                        style="min-width:150px;">
                                                        @foreach($poData as $index => $po)
                                                            <option value="{{ $po['po_no'] }}" data-total-qty="{{ $po['total_qty'] }}"
                                                                ...
                                                                class="{{ ($po['followed'] ?? false) ? 'po-followed' : '' }}" {{ $index === 0 ? 'selected' : '' }}>
                                        ...
                                        <td>
                                            <div class="request-whc-cell" data-item-id="{{ $item['id'] }}"
                                                data-outstanding="{{ $item['outstanding'] ?? 0 }}">
                                                @if(auth()->check() && auth()->user()->username !== 'guest' && (auth()->user()->isWarehouse() || auth()->user()->isMaster() || in_array(auth()->user()->username, ['whc', 'master', 'admin'])))
                                                    <input type="number"
                                                        class="form-control form-control-sm request-whc-input {{ $requestWhc !== null ? 'text-danger' : '' }}"
                                                        value="{{ $requestWhc !== null ? $requestWhc : '' }}" min="0"
                                                        max="{{ $item['outstanding'] ?? 0 }}" placeholder="0"
                                                        data-outstanding="{{ $item['outstanding'] ?? 0 }}"
                                                        style="width: 100px; display: inline-block;">
                                                @else
                                                    <div
                                                        class="form-control-plaintext text-end fw-semibold {{ $requestWhc !== null ? 'text-danger' : '' }}">
                                                ...
                                                @if($requestWhcEditedAt)
                                                    <div style="font-size: 0.75rem; color: #6c757d; margin-top: 4px;">
                                                        last edited
                                                        {{ strtolower(\Carbon\Carbon::parse($requestWhcEditedAt)->setTimezone('Asia/Jakarta')->format('M d, H:i')) }}
                                                    </div>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            <div class="request-whc-date-cell" data-item-id="{{ $item['id'] }}">
                                                @if(auth()->check() && auth()->user()->username !== 'guest' && (auth()->user()->isWarehouse() || auth()->user()->isMaster() || in_array(auth()->user()->username, ['whc', 'master', 'admin'])))
                                                    <input type="date" class="form-control form-control-sm request-whc-date-input"
                                                ...
                                                @if($requestWhcDateEditedAt)
                                                    <div class="last-edited-whc-date"
                                                        style="font-size: 0.75rem; color: #6c757d; margin-top: 4px;">
                                                        last edited
                                                        {{ strtolower(\Carbon\Carbon::parse($requestWhcDateEditedAt)->setTimezone('Asia/Jakarta')->format('M d, H:i')) }}
                                                    </div>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="text-end">
                                            <strong>{{ number_format($item['ending_balance'] ?? 0, 0, ',', '.') }}</strong>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            @if($sudahFollowEditedAt)
                                                <div style="font-size: 0.75rem; color: #6c757d; margin-top: 4px;">
                                                    last edited
                                                    {{ strtolower(\Carbon\Carbon::parse($sudahFollowEditedAt)->setTimezone('Asia/Jakarta')->format('M d, H:i')) }}
                                                </div>
                                            @endif
                                        </td>
                                        <td>
                                            ...
                                            @if($pengirimanTanggalEditedAt)
                                                <div style="font-size: 0.75rem; color: #6c757d; margin-top: 4px;">
                                                    last edited
                                                    {{ strtolower(\Carbon\Carbon::parse($pengirimanTanggalEditedAt)->setTimezone('Asia/Jakarta')->format('M d, H:i')) }}
                                                </div>
                                            @endif
                                        </td>
                                        <td>
                                            @if(!empty($item['imported_at']))
                                                <small
                                                    class="text-muted">{{ \Carbon\Carbon::parse($item['imported_at'])->setTimezone('Asia/Jakarta')->format('d/m/Y H:i') }}
                                                    WIB</small>
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif
                                        ...
                                        <td>
                                            <div class="note-cell" data-id="{{ $item['id'] }}">
                                                <div class="note-display">
                                                    <span
                                                        class="note-text">{{ !empty($item['note']) ? $item['note'] : '-' }}</span>
                                                    @if(auth()->check() && auth()->user()->username !== 'guest')
                                                        <button type="button" class="btn btn-sm btn-link p-0 edit-note-btn"
                                                            title="Edit Note">
                                                ...
                                                <div class="note-edit d-none">
                                                    <textarea class="form-control form-control-sm note-input" rows="2"
                                                        maxlength="500"
                                                        placeholder="Tulis note...">{{ $item['note'] ?? '' }}</textarea>
                                                    <div class="d-flex gap-1 mt-1">
                                                        <button type="button"
                                                            class="btn btn-sm btn-primary btn-save-note">Simpan</button>
                                                        <button type="button"
                                                            class="btn btn-sm btn-secondary btn-cancel-note">Batal</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="20" class="text-center text-muted">Belum ada item minim (semua item
                                            memiliki ending balance >= min atau outstanding = 0)</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control" name="outstanding" id="edit_outstanding" min="0"
                                    required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">ENDING BALANCE <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" name="ending_balance" id="edit_ending_balance"
                                    min="0" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">MAX <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" name="maximal_stock" id="edit_maximal_stock"
                                    min="0" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">ORDER POINT <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" name="order_point" id="edit_order_point" min="0"
                                    required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">MIN <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" name="minimal_stock" id="edit_minimal_stock"
                                    min="0" required>
                            </div>
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-control-plaintext fw-semibold" id="modal_item_code"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            -</div>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label text-muted small mb-1">Description</label>
                                        <div class="form-control-plaintext fw-semibold" id="modal_item_name"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            -</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label text-muted small mb-1">OUTSTANDING</label>
                                        <div class="form-control-plaintext text-end fw-semibold text-primary"
                                            id="modal_outstanding"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            0</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label text-muted small mb-1">Request WHC</label>
                                        <div class="form-control-plaintext text-end fw-semibold" id="modal_request_whc"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            -</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label text-muted small mb-1">Request Date WHC</label>
                                        <div class="form-control-plaintext text-end fw-semibold" id="modal_request_date_whc"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            -</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label text-muted small mb-1">ENDING BALANCE</label>
                                        <div class="form-control-plaintext text-end fw-semibold" id="modal_ending_balance"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            0</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label text-muted small mb-1">MAX</label>
                                        <div class="form-control-plaintext text-end fw-semibold" id="modal_maximal_stock"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            0</div>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label text-muted small mb-1">ORDER POINT</label>
                                        <div class="form-control-plaintext text-end fw-semibold" id="modal_order_point"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            0</div>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label text-muted small mb-1">Supplier Name</label>
                                        <div class="form-control-plaintext fw-semibold" id="modal_supplier_name"
                                            style="min-height: 38px; padding: 0.375rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            -</div>
                                    </div>
                                </div>
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-12" id="modal_po_no_row" style="display: none;">
                                        ...
                                        <select class="form-select form-select-lg" id="modal_po_no_select" name="po_no"
                                            required>
                                        </select>
                                    ...
                                    <div class="col-12" id="modal_po_no_single_row" style="display: none;">
                                        ...
                                        <div class="form-control-plaintext fw-semibold" id="modal_po_no_single"
                                            style="min-height: 48px; padding: 0.5rem 0.75rem; background-color: #f8f9fa; border-radius: 0.375rem;">
                                            -</div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" class="form-control form-control-lg"
                                            id="modal_qty_akan_dikirim" name="qty_akan_dikirim" min="0" max="0" required
                                            placeholder="Masukkan jumlah QTY">
                                        <div class="mt-2">
                                            <span class="badge bg-info" id="modal_qty_max_info">
                                                <i data-feather="alert-circle" class="me-1"
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control form-control-lg"
                                            id="modal_pengiriman_tanggal" name="pengiriman_tanggal" placeholder="">
                                        <div class="form-check mt-2">
                                            <input class="form-check-input" type="checkbox"
                                                id="modal_tanggal_belum_ditentukan" name="tanggal_belum_ditentukan">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.open-follow-modal-btn').forEach(btn => {
                btn.addEventListener('click', function () {
                    ...
                    const requestWhcDate = this.getAttribute('data-request-whc-date') || '';
                    document.getElementById('modal_request_date_whc').textContent = requestWhcDate !== '' ? requestWhcDate : '-';
                    ...
                    if (dateInput) {
                        ...
                        if (typeof flatpickr !== 'undefined') {
                            ...
                            if (!checkbox || !checkbox.checked) {
                                ...
                                // Force remove readonly after initialization just in case
                                setTimeout(() => { dateInput.removeAttribute('readonly'); }, 100);
                            }
                        }
                    }
