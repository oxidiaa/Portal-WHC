                            ...
                            <tbody>
                                @forelse($poItems as $index => $item)
                                    <tr data-item-id="{{ $item['id'] }}">
                                        ...
                                        <td class="text-end">
                                            {{ number_format($item['scheduled_receipt_qty'] ?? 0, 0, ',', '.') }}</td>
                                        <td>{{ $item['po_no'] ?? '-' }}</td>
                                        <td>
                                        ...
                                        <td>
                                            @if(auth()->check() && (auth()->user()->isMaster() || auth()->user()->isWarehouse() || in_array(auth()->user()->username, ['master', 'admin', 'whc'])))
                                                <form action="{{ route('data_po.destroy', $item['id']) }}" method="POST"
                                                    class="d-inline"
                                                    onsubmit="return confirm('Yakin ingin menghapus item PO ini? Item outstanding yang terkait juga akan dihapus.');">
                                                    @csrf
                                                    @method('DELETE')
