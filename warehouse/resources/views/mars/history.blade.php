...
@section('content')
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <div class="d-flex gap-2">
                            @if(auth()->check() && auth()->user()->username !== 'guest' && (auth()->user()->isMaster() || auth()->user()->isWarehouse() || in_array(auth()->user()->username, ['master', 'admin', 'whc'])))
                            ...
                            <table class="table table-striped table-bordered">
                                <thead class="table-dark" style="position: sticky; top: 0; z-index: 10;">
                                    <tr>
                                        ...
                                        @if(auth()->check() && (auth()->user()->isMaster() || auth()->user()->isWarehouse() || in_array(auth()->user()->username, ['master', 'admin', 'whc'])))
                                            <th>Action</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="10" class="text-center text-muted py-4">Belum ada riwayat kedatangan barang</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
    ...
    <!-- Single Shared Edit History Modal -->
    <div class="modal fade" id="sharedEditHistoryModal" tabindex="-1" aria-labelledby="sharedEditHistoryModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="sharedEditHistoryModalLabel">Edit Item History</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="sharedEditHistoryForm" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Item Code</label>
                            <input type="text" class="form-control" id="modal_edit_item_code" name="item_code" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Item Name</label>
                            <input type="text" class="form-control" id="modal_edit_item_name" name="item_name" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Supplier name</label>
                            <input type="text" class="form-control" id="modal_edit_supplier_name" name="supplier_name">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Sched. receipt qty.</label>
                            <input type="number" class="form-control" id="modal_edit_scheduled_receipt_qty" name="scheduled_receipt_qty" min="0">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">PO No.</label>
                            <input type="text" class="form-control" id="modal_edit_po_no" name="po_no">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Jumlah Item yang Datang</label>
                            <input type="number" class="form-control" id="modal_edit_jumlah_item_datang" name="jumlah_item_datang" min="0" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Pengiriman Tanggal</label>
                            <input type="date" class="form-control" id="modal_edit_pengiriman_tanggal" name="pengiriman_tanggal">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Tanggal Kedatangan</label>
                            <input type="date" class="form-control" id="modal_edit_arrival_date" name="arrival_date" required>
                        </div>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.edit-history-btn').forEach(btn => {
                btn.addEventListener('click', function () {
                    ...
                    const form = document.getElementById('sharedEditHistoryForm');
                    if (form) {
                        form.action = `/history/${id}`;
                    }

                    const codeEl = document.getElementById('modal_edit_item_code');
                    const nameEl = document.getElementById('modal_edit_item_name');
                    const supplierEl = document.getElementById('modal_edit_supplier_name');
                    const qtyEl = document.getElementById('modal_edit_scheduled_receipt_qty');
                    const poEl = document.getElementById('modal_edit_po_no');
                    const arrivalQtyEl = document.getElementById('modal_edit_jumlah_item_datang');
                    const shipDateEl = document.getElementById('modal_edit_pengiriman_tanggal');
                    const arrivalDateEl = document.getElementById('modal_edit_arrival_date');

                    if (codeEl) codeEl.value = this.dataset.itemCode || '';
                    if (nameEl) nameEl.value = this.dataset.itemName || '';
                    if (supplierEl) supplierEl.value = this.dataset.supplierName || '';
                    if (qtyEl) qtyEl.value = this.dataset.scheduledReceiptQty || 0;
                    if (poEl) poEl.value = this.dataset.poNo || '';
                    if (arrivalQtyEl) arrivalQtyEl.value = this.dataset.jumlahItemDatang || 0;
                    if (shipDateEl) shipDateEl.value = this.dataset.pengirimanTanggal || '';
                    if (arrivalDateEl) arrivalDateEl.value = this.dataset.arrivalDate || '';

                    const modalEl = document.getElementById('sharedEditHistoryModal');
                    if (modalEl) {
