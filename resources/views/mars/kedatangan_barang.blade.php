                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <div class="d-flex gap-2">
                            <form action="{{ route('kedatangan_barang.importExcel') }}" method="POST"
                                enctype="multipart/form-data" class="d-inline" id="importExcelForm">
                                @csrf
                                <input type="hidden" name="arrival_date" id="arrival_date_hidden">
                                <input type="file" name="excel_file" accept=".xlsx,.xls" id="kedatangan_excel_file"
                                    style="display: none;" required>
                                <button type="button" class="btn btn-success"
                                    onclick="document.getElementById('kedatangan_excel_file').click()">
                                    <i data-feather="upload"></i> Upload Excel Kedatangan Barang
                                </button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-info">
                        <strong>Info:</strong> Import Excel akan mengurangi outstanding dari Item Outstanding, menambah
                        ending balance, dan memindahkan item yang datang ke History. Format Excel sama dengan Data PO: Item
                        CD, Item name, Supplier name, Sched. receipt qty, PO No. Item dengan PO No. yang tidak sesuai atau
                        Qty melebihi scheduled receipt qty akan ditandai dengan warna merah.
                    </div>
                        ...
                        <div class="card border border-success mb-4">
                            <div class="card-body">
                                ...
                                <p class="text-muted mb-3">
                                    Tanggal kedatangan:
                                    <strong>{{ isset($importSummary['arrival_date']) ? date('d/m/Y', strtotime($importSummary['arrival_date'])) : '-' }}</strong>
                                </p>
                                            ...
                                            @foreach(($importSummary['items'] ?? []) as $index => $summary)
                                                ...
                                                <tr>
                                                        ...
                                                        @if(isset($summary['po_no']) && !empty($summary['po_no']))
                                                            @if(isset($summary['po_validation']) && $summary['po_validation'] === 'invalid')
                                                                <span class="badge bg-danger"
                                                                    title="PO No. tidak sesuai atau Qty melebihi scheduled receipt qty">
                                                                    {{ $summary['po_no'] }} <i data-feather="alert-circle"
                                                    ...
                                                    <td class="text-end">
                                                        @if(isset($summary['po_validation']) && $summary['po_validation'] === 'invalid')
                                                            <span
                                                                class="text-danger fw-bold">{{ number_format($summary['arrived_qty'] ?? 0, 0, ',', '.') }}</span>
                                                        @else
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title" id="{{ $modalId }}Label">Edit Item
                                                                    Kedatangan</h5>
                                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                    aria-label="Close"></button>
                                                            </div>
                                                            <form
                                                                action="{{ route('history.update', $summary['history_id'] ?? '') }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('PUT')
                                                                <input type="hidden" name="redirect_to" value="kedatangan_barang">
                                                                <div class="modal-body">
                                                                    <div class="mb-3">
                                                                        ...
                                                                        <input type="text" class="form-control" name="item_code"
                                                                            value="{{ $summary['item_code'] ?? '' }}" required>
                                                                    </div>
                                                                    <div class="mb-3">
                                                                        <label class="form-label">Item Name</label>
                                                                        <input type="text" class="form-control" name="item_name"
                                                                            value="{{ $summary['item_name'] ?? '' }}" required>
                                                                    </div>
                                                                    <div class="mb-3">
                                                                    ...
                                                                    <div class="mb-3">
                                                                        ...
                                                                        <input type="number" class="form-control"
                                                                            ...
                                                                            value="{{ $summary['arrived_qty'] ?? 0 }}" min="0"
                                                                            required>
                                                                    </div>
                                                                    <div class="mb-3">
                                                                        <label class="form-label">Tanggal Kedatangan</label>
                                                                        <input type="date" class="form-control" name="arrival_date"
                                                                            value="{{ isset($summary['arrival_date']) ? date('Y-m-d', strtotime($summary['arrival_date'])) : date('Y-m-d') }}"
                                                                            required>
                                                                    </div>
                                                                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">Tanggal Kedatangan</label>
                        <input type="date" class="form-control" id="import_arrival_date" value="{{ date('Y-m-d') }}"
                            required>
                        <small class="text-muted">Tanggal ini akan digunakan untuk semua item yang datang melalui import
                            Excel.</small>
                    </div>
                </div>
                <div class="modal-footer">
